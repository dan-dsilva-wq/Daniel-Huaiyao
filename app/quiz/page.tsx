  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-stone-50 to-zinc-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      <main className="relative z-10 max-w-2xl mx-auto px-4 py-6 sm:py-12 pb-safe">
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'your-questions' && (
            <motion.div
              ...
              className="space-y-4"
            >
              {/* Add Question Form - NOW AT THE TOP */}
              {showAddForm && (
                <motion.div
                  initial={{ opacity: 0, y: -20 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="bg-white/70 dark:bg-gray-800/70 backdrop-blur rounded-xl p-4 shadow-sm border-2 border-indigo-200 dark:border-indigo-800"
                >
                  ...
                  <label className="flex items-center gap-3 mb-4 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg cursor-pointer">
                    <input
                      type="checkbox"
                    ...
                    />
                    <div>
                      <span className="font-medium text-gray-800 dark:text-white">Multiple choice</span>
                    ...
                    {newOptions.map((option, idx) => (
                      <div key={idx} className="relative flex gap-2">
                        <input
                          ...
                          className={`flex-1 px-4 py-3 pr-10 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300 dark:bg-gray-700 dark:text-white ${
                            isMultipleChoice
                        ...
                        {newOptions.length > 2 && (
                          <button
                            ...
                            className="w-10 h-10 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center justify-center transition-colors flex-shrink-0"
                          >
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                            </svg>
                  ...
                  {/* Two-way toggle */}
                  <label className="flex items-center gap-3 mb-4 p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg cursor-pointer">
                    <input
                      type="checkbox"
                      checked={isTwoWay}
              ...
              {myQuestions.filter((q) => !q.pending_setup).length === 0 && !showAddForm ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600 dark:text-gray-400 text-lg mb-2">No questions yet!</p>
                  <p className="text-gray-400 dark:text-gray-500 text-sm mb-6">Add questions about yourself for {partnerName} to answer</p>
                  <button
                    onClick={() => setShowAddForm(true)}
                    className="px-6 py-3 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white rounded-xl font-medium shadow-lg hover:shadow-xl transition-all inline-flex items-center gap-2"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* Floating Add Button - visible on BOTH tabs when form is closed */}
        <AnimatePresence>
          {!showAddForm && (
            <motion.button
              ...
              onClick={() => {
                setActiveTab('your-questions');
                setShowAddForm(true);
              }}
              className="fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white rounded-full shadow-lg hover:shadow-xl transition-all flex items-center justify-center z-50"
            >
