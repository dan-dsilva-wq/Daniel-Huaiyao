...
async function main(): Promise<void> {
  ...
  const trainDeepScript = resolveScriptPath('scripts/hive/train-deep.ts');
  const evalScript = resolveScriptPath('scripts/hive/eval.ts');
...
function printHelpAndExit(): never {
  console.log('Usage: npm run hive:train:deep:eval -- [train args] [eval args]');
  console.log('');
  console.log('Behavior:');
  console.log('  1) Runs train-deep.ts with hive:train:deep preset args, then your train overrides');
  console.log('  2) Runs hive:eval with default eval args, plus eval overrides');
  console.log('');
  console.log('Train preset defaults:');
  console.log(`  ${DEFAULT_DEEP_TRAIN_ARGS.join(' ')}`);
  console.log('');
  console.log('Train args:');
  console.log('  Any non --eval-* args are forwarded to hive:train:deep');
  console.log('');
