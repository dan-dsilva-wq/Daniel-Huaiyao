...
const DEFAULT_OPTIONS: DeepTrainOptions = {
  ...
  keepDataset: false,
  datasetOut: null,
  checkpointPath: '.hive-cache/deep-training.ckpt',
  resume: true,
  warmStart: true,
  saveEvery: 1,
  earlyStopPatience: 6,
  ...
  replayEnabled: true,
  replayPath: '.hive-cache/deep-replay-buffer.json',
  replayMaxRuns: 6,
...
async function main(): Promise<void> {
  ...
  const rawDatasetPath = options.datasetOut
    ...
    : path.resolve(process.cwd(), '.hive-cache', `deep-dataset-${Date.now()}.json`);
  ...
  logStage('setup', `Self-play limits | maxTurns=${options.maxTurns} noProgressDraw=${options.noCaptureDrawMoves}`);
  logStage(
    'setup',
  ...
  logStage('self-play', 'Generating dataset with self-play...');

  const trainModelScript = path.resolve(process.cwd(), 'scripts/hive/train-model.ts');
  const selfPlayArgs = [
    '--import',
    'tsx',
    trainModelScript,
  ...
  logStage('deep-train', 'Training deep neural net (PyTorch)...');
  const deepScriptPath = path.resolve(process.cwd(), 'scripts/hive/train-model-deep.py');
  const pythonArgs = [
    deepScriptPath,
    '--dataset',
...
function readOptionalDifficulty(value: unknown): HiveComputerDifficulty | undefined {
...
function printUsageAndExit(): never {
  console.log('Usage: npm run hive:train:deep -- [options]');
  console.log('');
  ...
  console.log('  --max-turns <n>       Max turns per game (default: 260)');
  console.log('  --no-capture-draw <n> Draw when no queen-pressure progress for N moves (default: 90, 0 disables)');
  ...
  console.log('  --checkpoint <path>   Checkpoint file for resume (default: .hive-cache/deep-training.ckpt)');
  ...
  console.log('  --replay-path <path>  Replay buffer dataset path (default: .hive-cache/deep-replay-buffer.json)');
